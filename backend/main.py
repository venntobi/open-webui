...
async def inspect_websocket(request: Request, call_next):
    # Skip WebSocket connections entirely
    if request.scope.get("type") == "websocket":
        return await call_next(request)

    # Original processing for HTTP requests
    log.debug(f"Inspect middleware - Path: {request.url.path}")
    try:
        response = await call_next(request)
        return response
    except Exception as e:
        ...
        # Any unhandled exception in the middleware gets logged and returns 500
        log.error(f"Middleware fuckingerror in inspect_websocket: {str(e)}")
        return JSONResponse(
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "http://10.3.0.158:5173", "http://192.168.2.78:5173"],
